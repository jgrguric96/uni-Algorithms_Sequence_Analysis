...
def forward(X,A,E):
    ...
    # Middle columns
    # Viterbi - replaced max with sum to be appropriate w.r.t forward algorithm logic
    for i, s in enumerate(X):
        for l in emittingStates:
    ...
    # Last column
    # Same as above. Viterby is pretty similar to it.
    term = 0
    for k in allStates:
        term += F[k][i + 1] * A[k]['E']
    if term > F['E'][-1]:
...
def backward(X,A,E):
    ...
    #####################
    # Remaining columns
    # An inverse of the forward algorithm implementation above.
    for i in range(L-2, 0, -1):
        s = X[i-1]
        for k in allStates:
            terms = [B[l][i] * A[k][l] * E[l][s] for l in emittingStates]
            B[k][i - 1] = sum(terms)
...
def baumwelch(set_X,A,E):
    ...
    for X in set_X:
        ...
        #  Following the formula for calculating new emissions.
        L = len(X) + 2
        for l in emittingStates:
            for i in range(1, L - 1):
                new_E[l][X[i - 1]] += (F[l][i] * B[l][i]) / P

        # Ksi = (ft(i)*aij*ej(t+1)*bt+1(j))/P
        # A_prime[K][L] is a K * K matrix. Cannot have L be only emitting states.
        # Nvm im dumb. Formula doesnt work with E[l] on k = 'E'. It will just multiply with 0.
        # This implementation follows formula given in presentations.
        for k in allStates:
            for l in emittingStates:
                for i in range(1, L-1):
                    new_A[k][l] += (F[k][i-1]*A[k][l]*E[l][X[i - 1]]*B[l][i])/P
            new_A[k]['E'] += (F[k][i] * A[k]['E']) / P


        
    # Outside the for loop: Maximization
    # Normalize row sums to 1 (except for one row in the Transition matrix!)
    # This is step 3 according to presentation.
    for i in allStates:
        if i == 'E':
            continue
        sum_A = sum(new_A[i].values())
        for j in allStates:
            new_A[i][j] = new_A[i][j] / sum_A

    for i in emittingStates:
        sum_E = sum(new_E[i].values())
        for j in new_E[i].keys():
            new_E[i][j] = new_E[i][j] / sum_E
